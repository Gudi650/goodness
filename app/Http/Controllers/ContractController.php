...
class ContractController extends Controller
{
    ...
    {
        $contract = Contract::with('company', 'manager')->findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        $signedPath = $contract->contract_signed_document_path;
        $signedName = $contract->contract_signed_document_name;
        if ($request->hasFile('contract_signed_document')) {
            $signed = $request->file('contract_signed_document');
            $signedPath = $signed->store('contracts/signed', 'public');
            $signedName = $signed->getClientOriginalName();
        ...
        $supportingDocs = $contract->contract_supporting_docs;
        if ($request->hasFile('contract_supporting_docs')) {
            $supportingDocs = [];
            foreach ($request->file('contract_supporting_docs') as $document) {
                $supportingDocs[] = [
        ...
        if ($request->expectsJson() || $request->ajax()) {
            return response()->json([
                'message' => "Contract '{$contract->contract_number}' updated successfully.",
                'contract_id' => $contract->id,
                'contract_number' => $contract->contract_number,
            ]);
        }

        return redirect()->back()->with('success', "Contract '{$contract->contract_number}' updated successfully.");
    }
    ...
    public function destroy($id)
    {
        ...
        if (request()->expectsJson() || request()->ajax()) {
            return response()->json([
                'message' => "Contract '{$contractNumber}' deleted successfully.",
            ]);
        }

        return redirect()->back()->with('success', "Contract '{$contractNumber}' deleted successfully.");
    }
}
