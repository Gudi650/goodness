        ...
        <table class="min-w-full">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($equityData as $index => $equity)
                    ...
                    <!-- Dropdown row -->
                    <tr class="hidden" id="dropdown-{{ $index }}">
                        <td colspan="7" class="px-4 py-3">
                            <div class="bg-white border border-slate-200 rounded-lg shadow-sm p-4 text-sm text-slate-700">
                                ...
                                <ul class="space-y-1">
                                    <li><strong>Shareholder:</strong> {{ $equity->shareholder }}</li>
                                    <li><strong>Company:</strong> {{ $equity->company->name ?? 'N/A' }}</li>
                                    <li><strong>Equity Type:</strong> Common Stock</li>
                                    <li><strong>Shares:</strong> {{ number_format($equity->shares) }}</li>
                                    <li><strong>Ownership:</strong> {{ number_format($ownershipPercent, 1) }}%</li>
                                    <li><strong>Status:</strong> Active</li>
                                    <li><strong>Share Value:</strong> ${{ number_format($equity->share_value ?? 0, 2) }}</li>
                                    <li><strong>Value Held:</strong> ${{ number_format($equity->value_held ?? 0, 2) }}</li>
                                    @if($equity->notes)
                                        <li><strong>Notes:</strong> {{ $equity->notes }}</li>
                                    @endif
                                </ul>
                            </div>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-sm text-slate-500">
                            No equity distribution records found.
                        </td>
                    </tr>
                @endforelse
            </tbody>
