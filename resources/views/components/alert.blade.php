<div id="alertContainer" class="fixed bottom-6 right-6 z-50 flex flex-col gap-2 pointer-events-none"></div>
...
<script>
  window.showAlert = function(type, message) {
    ...
    const alertHTML = `
      <div id="${alertId}" class="flex items-start gap-3 px-4 py-3 rounded-lg shadow-md text-sm max-w-sm ${config.bg} ${config.border} border ${config.text} pointer-events-auto animate-in fade-in slide-in-from-right">
        ${config.icon}
        <span class="flex-1">${message}</span>
        <button onclick="dismissAlert('${alertId}')" class="flex-shrink-0 text-current hover:opacity-70 transition-opacity">
    ...
    alertElement.dataset.timeout = timeout;
  };

  window.dismissAlert = function(alertId) {
    const alertElement = document.getElementById(alertId);
    if (alertElement) {
      clearTimeout(parseInt(alertElement.dataset.timeout));
      alertElement.classList.add('opacity-0', 'transition-opacity', 'duration-300');
