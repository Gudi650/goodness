<div id="topbar"
    ...
    <div class="flex items-center gap-2 lg:gap-4 flex-1 min-w-0">
        <button onclick="toggleSidebar()"
            class="lg:hidden p-2 hover:bg-slate-100 rounded-md text-slate-600 flex-shrink-0">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
            </svg>
        </button>
        <h1 id="pageTitle" class="text-sm lg:text-base font-semibold text-slate-800 font-display truncate">Dashboard
        </h1>
    </div>
    ...
    <div class="flex items-center gap-2 lg:gap-4 flex-shrink-0">
        ...
        @else
            ...
            <div
                class="hidden sm:flex items-center gap-2 px-3 py-1.5 border border-slate-200 rounded-md bg-slate-50 text-xs lg:text-sm text-slate-700 max-w-xs truncate">
        ...
        <div class="relative hidden sm:block">
            <button
                class="p-2 hover:bg-slate-100 rounded-md text-slate-500 hover:text-slate-800 transition-colors flex-shrink-0">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex items-center gap-2 lg:gap-3 pl-2 lg:pl-4 border-l border-slate-200">
            ...
            <a
                href="{{ route('settings') }}"
                class="flex items-center gap-1 lg:gap-1.5 text-xs lg:text-sm text-slate-500 hover:text-brand-600 transition-colors px-2 lg:px-4 border-l border-slate-200 flex-shrink-0"
            ...
            <a href="{{ route('settings') }}"
                class="flex items-center gap-1 text-xs text-slate-500 hover:text-brand-600 transition-colors px-2 border-l border-slate-200 flex-shrink-0 
                block lg:hidden"
                title="Manage account settings">
                {{-- Settings icon --}}
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {{-- Logout Form - only visible on mobile screens, submits POST request to /logout endpoint --}}
            <form action="{{ route('logout') }}" method="POST" class="flex items-center block lg:hidden">
                {{-- CSRF Token - required by Laravel for security --}}
                @csrf

                {{-- Logout Button - styled as a link but submits the form --}}
                <button type="submit"
                    class="flex items-center gap-1 text-xs text-slate-500 hover:text-red-600 transition-colors pl-2 border-l border-slate-200 flex-shrink-0"
                    title="Log out of your account">
                    {{-- Logout icon --}}
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
